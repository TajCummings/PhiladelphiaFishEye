...
## 🛠️ Database Setup
We use DBML to manage our database schema. 
You can find the blueprint file in `docs/database.dbml`.
To visualize it, copy the contents into [dbdiagram.io](https://dbdiagram.io).
...
//
--docker exec -i tacotracker-db psql -U tacouser -d tacotracker < docs/schema.sql

Note that I decided to use `psql` instead of `dotnet ef database update`. Specificaly I used the dbml file -  Docs: https://dbml.dbdiagram.io/docs - to generate the schema SQL. Then 
docker exec -i tacotracker-db psql -U tacouser -d tacotracker < docs/schema.sql to populate the db.
After, used Entitify framework to create the database context and models.
